...
Remember: Always end with exactly six words, use simple language, and guide step by step.`
  ...
  const summarizePlot = async (messageHistory?: Message[]) => {
    try {
      ...
      if (settingMatch && settingMatch[1].trim()) {
        ...
        const newSetting = settingMatch[1].trim().replace(/^setting[：:]\s*/i, "").trim()
        if (newSetting && newSetting !== plotData.setting) {
      ...
      if (conflictMatch && conflictMatch[1].trim()) {
        ...
        const newConflict = conflictMatch[1].trim().replace(/^conflict[：:]\s*/i, "").trim()
        if (newConflict && newConflict !== plotData.conflict) {
      ...
      if (goalMatch && goalMatch[1].trim()) {
        // 去掉可能的"goal:"前缀和多余空格
        const newGoal = goalMatch[1].trim().replace(/^goal[：:]\s*/i, "").trim()
        if (newGoal && newGoal !== plotData.goal) {
  ...
  return (
    <div className="min-h-screen py-8 px-6 bg-gradient-to-br from-blue-100 via-cyan-50 via-purple-50 to-pink-50">
            ...
            <div className="bg-gradient-to-br from-white to-purple-50 rounded-2xl p-8 border-2 border-purple-200 shadow-2xl">
              ...
              {summaryDone && (
                <Button
                  ...
                  size="lg"
                  className="w-full mt-6 bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 hover:from-indigo-700 hover:via-purple-700 hover:to-pink-700 text-white border-0 shadow-xl py-6 text-lg font-bold animate-pulse"
                >
              ...
              <div className="space-y-4">
                <div className={`transition-all duration-500 ${updatingFields.has("setting") ? "animate-pulse scale-105" : ""}`}>
                  ...
                  <div className={`p-3 rounded-xl border-2 transition-all duration-500 ${
                    ...
                    <p className={`text-sm font-bold transition-all duration-500 ${
                      plotData.setting ? "text-blue-800" : "text-gray-400"
                    }`}>
                      {plotData.setting || "Not set"}
                    </p>
                  </div>
                </div>
                <div className={`transition-all duration-500 ${updatingFields.has("conflict") ? "animate-pulse scale-105" : ""}`}>
                  ...
                  <div className={`p-3 rounded-xl border-2 transition-all duration-500 ${
                    ...
                    <p className={`text-sm font-bold transition-all duration-500 ${
                      plotData.conflict ? "text-purple-800" : "text-gray-400"
                    }`}>
                      {plotData.conflict || "Not set"}
                    </p>
                  </div>
                </div>
                <div className={`transition-all duration-500 ${updatingFields.has("goal") ? "animate-pulse scale-105" : ""}`}>
                  ...
                  <div className={`p-3 rounded-xl border-2 transition-all duration-500 ${
                    ...
                    <p className={`text-sm font-bold transition-all duration-500 ${
                      plotData.goal ? "text-pink-800" : "text-gray-400"
                    }`}>
                      {plotData.goal || "Not set"}
                    </p>
                  </div>
                </div>
              </div>
