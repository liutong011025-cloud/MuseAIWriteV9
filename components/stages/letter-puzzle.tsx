...
// 打乱数组顺序的函数 - 使用确定性的打乱（基于索引）避免 SSR 不一致
const shuffleArray = <T,>(array: T[]): T[] => {
  const shuffled = [...array]
  // 使用 Fisher-Yates 洗牌算法，但使用固定种子避免 SSR/CSR 不一致
  // 使用数组长度作为种子的一部分，确保每次组件的打乱是一致的
  let seed = array.length * 7 // 固定种子
...
export default function LetterPuzzle({
  ...
  // 在客户端初始化时打乱顺序 - 每次 sections 变化都重新打乱
  useEffect(() => {
    if (sections.length > 0) {
      setShuffledSections(shuffleArray(sections.map((_, index) => index)))
    }
