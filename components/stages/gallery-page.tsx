...
import { BookOpen, Book, Mail, FileText, ChevronDown, ChevronRight, Sparkles } from "lucide-react"
...
export default function GalleryPage({ onBack }: GalleryPageProps) {
  ...
  const extractArticles = (): Article[] => {
    ...
    interactions
      .filter(i => i.review && (i.stage === 'bookReviewComplete' || i.review.trim().length > 0))
      .forEach((i, idx) => {
        const review = typeof i.review === 'string' ? i.review : (i.output?.review || '')
        if (review.trim()) {
          articles.push({
            id: `review-${i.user_id}-${i.timestamp}`,
            title: `${i.reviewType || 'Book'} Review: ${i.bookTitle || 'Unknown Book'}`,
            author: i.user_id,
            type: 'bookReview',
            content: review,
            timestamp: i.timestamp,
            bookTitle: i.bookTitle,
            coverUrl: i.bookCoverUrl,
          })
        }
      })
